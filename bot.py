...
async def forward_second_rule(message):
    # Ignore bot messages
    if message.author.bot:
        return

    # Only process messages from user 2
    if message.author.id != USER_ID_2:
        return

    # Check if the trigger role 2 is mentioned
    if not any(role.id == TRIGGER_ROLE_ID_2 for role in message.role_mentions):
        return

    destination_channel = bot.get_channel(DESTINATION_CHANNEL_ID_2)

    if not destination_channel:
        print(f"Error: Could not find destination channel {DESTINATION_CHANNEL_ID_2}")
        return
    ...
    try:
        ...
        print(f"Error forwarding message (rule 2): {e}")
        try:
            await message.add_reaction("❌")
        except:
            pass
